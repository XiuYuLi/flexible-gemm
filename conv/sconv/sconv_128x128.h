...
#define sconvx_128x128(add_bias,suffix)   \
__global__ void dk_sconvx_128x128##suffix(\
          char *              d_c   ,\
    const char * __restrict__ d_a   ,\
    const char * __restrict__ d_b   ,\
    const float* __restrict__ d_bias,\
    ...
    unsigned int sv=v<qnc?v:(qnc-1); \
    unsigned int cxy=cx*cy;          \
    unsigned int idx=su%cxy;         \
    unsigned int x=(bx<<6)+(slot_x<<5)+lane;\
    unsigned int y=(by<<7)+(slot_y<<5);     \
    d_a+=bz*pnc*lda+((((su/cxy)*ax+(idx/cy))*ay+(idx%cy))<<2);\
    d_b+=(bz*qnc+sv)*ldb+(vtid<<4);\
    ...
    *((float*)&sst_base[0x1600])=q.w;\
    __syncthreads();\
    if(add_bias){ if((v<qnc)&(vtid==0)){ s_bias[utid]=d_bias[bz*qnc+v]; } }\
    SZERO64(c)\
    b[0]=*((float4*)&bsld[0x00]);\
    a[0]=*((float4*)&asld[0x00]);\
    b[1]=*((float4*)&bsld[0x40]);\
...
sconvx_128x128(0,)
sconvx_128x128(0,_relu)
sconvx_128x128(1,_bias)
sconvx_128x128(1,_bias_relu)